...
export class CartPage {
  ...
  public async toHaveItems(expectedItems: readonly Product[]): Promise<void> {
    ...
    await expect(
      rows,
      `Expected ${expectedItems.length} cart item row(s).`,
    ...
    for (const [index, expectedItem] of expectedItems.entries()) {
      ...
      await expect(
        row,
        `Expected cart row ${index + 1} to contain "${expectedItem.name}".`,
      ).toContainText(expectedItem.name);
      await expect(
        row,
        `Expected cart row ${index + 1} to contain price ${expectedItem.price}.`,
      ).toContainText(String(expectedItem.price));
    }
  }
